...
def recommend_geo(
    ...
    if (
        not user
        or getattr(user, "latitude", None) is None
        or getattr(user, "longitude", None) is None
    ):
    ...
    rated_subq = UserMusicRating.select(UserMusicRating.music).where(
        UserMusicRating.user == user_id
    )
    ...
    if method == "haversine":
        ...
        try:
            ...
            q = (
                Music.select(Music, earth_expr.alias("dist_m"))
                ...
                out.append(
                    {
                        ...
                        "artist_id": getattr(row, "artist_id", None),
                        "distance_km": (
                            (float(dist_m) / 1000.0) if dist_m is not None else None
                        ),
                        "posted_at": getattr(row, "posted_at", None),
                    }
                )
            ...
            logger.exception(
                "earth_distance approach failed — falling back to haversine."
            )
            method = "haversine"

    if (
        method == "haversine" and not out
    ):  # Only run haversine if earth_distance didn't already provide results or failed
        sample_q = (
            Music.select(
                Music.id,
                Music.title,
            ...
            .join(User, on=(Music.artist == User.id))
            .where(
                (User.latitude.is_null(False))
                & (User.longitude.is_null(False))
                & (Music.id.not_in(rated_subq))
            )
        ...
        for row in sample_q:
            ...
            try:
                ...
                if haversine_km:
                    ...
                    dist_km = float(
                        backend.utils.geo._haversine_km(ulat, ulon, rlat, rlon)
                    )
            except (TypeError, ValueError):
                logger.debug(
                    "Invalid lat/lon for candidate %s — skipping",
                    getattr(row, "id", None),
                )
                continue
            else:
                ...
                dist_km = float(
                    backend.utils.geo._haversine_km(
                        float(user.latitude),
