...
def mock_user(mock_models):
    ...
    mocked_user = MagicMock(
        id=1, latitude=34.052235, longitude=-118.243683, name="Mocked User"
    )
...
def mock_music_query(mock_models):
    ...
    (
        Music.select.return_value.join.return_value.where.return_value.order_by.return_value.limit.return_value
    ) = mock_result
...
def test_recommend_geo_user_not_found(mock_pop, mock_models):
    ...
    result = recommend_geo(
        User=User, Music=Music, UserMusicRating=UserMusicRating, user_id=999
    )

    mock_pop.assert_called_once_with(
        User=User, Music=Music, UserMusicRating=UserMusicRating, user_id=999, limit=10
    )
    assert result == [{"id": 1}]
...
def test_recommend_geo_user_no_location(mock_pop, mock_models):
    ...
    mock_pop.return_value = [{"id": 1}]
    result = recommend_geo(
        User=User, Music=Music, UserMusicRating=UserMusicRating, user_id=1
    )
...
def test_recommend_geo_within_radius(
    mock_models, mock_user, mock_music_query, monkeypatch
):
    User, Music, UserMusicRating = mock_models
...
def test_recommend_geo_outside_radius(
    mock_models, mock_user, mock_music_query, monkeypatch
):
    User, Music, UserMusicRating = mock_models
    UserMusicRating.select.return_value.where.return_value = []
    ...
    with patch(
        "backend.services.geo.recommend_popular", return_value=[{"id": 200}]
    ) as mock_pop:
        result = recommend_geo(
            User=User,
            Music=Music,
...
def test_recommend_geo_fill_limit(mock_pop, mock_models, mock_user):
    ...
    near = MagicMock(
        id=101, title="Close Song", latitude=34.052235, longitude=-118.243683
    )
